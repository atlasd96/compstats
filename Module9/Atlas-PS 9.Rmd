...
## b) 
Next, we use permutation tests to examine the hypothesis that htere is no difference in mean survival time.
```{r}
...
# Problem 4
## a)
We draw 100 random points from the standard normal distribution.
```{r}
set.seed(73)
...
The mean of the data is .069.

## b) 
Next, we resample the data and calculate the means of each sample.
```{r}
sapply(seq(1, 10), function(i) mean(sample(data, length(data), replace=TRUE)))
```
