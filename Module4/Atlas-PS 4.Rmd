...
monte_carlo_integration <- function(f, a, b, n, plot=FALSE){
  ...
  if(plot){
    ...
    plot(seq(a, b, .01), f(seq(a, b, .01)), 'l', lwd=5, col='blue', xlab='X', ylab=TeX("$f(x)$"))
    points(x_points, y_points, col= as.factor(ifelse(f(x_points) <= y_points, 0, 1)), lwd=.25)
...
print(paste0("It took ", total, " random draws to accumulate 10000 points, leading to efficiency ", round(n / total, 3)))
...
However, if we were to want to use a squeezing function for this problem, we could use a piecewise function 
\[
s(x) = 
\begin{cases}
0 & x < -.57 \\
...
We plot the densities again with the squeezing function on top. 
```{r}
plot(seq(-10, 10, .01), dnorm(seq(-10, 10, .01), 0, 1), 'l', 
     ylim=c(0, .5), 
     ...
     xlab = 'X', ylab='f(x)')
lines(seq(-10, 10, .01), dnorm(seq(-10, 10, .01), 1, 2) / .4, 'l')
s <- function(x) ifelse(x < -.84 | x > .84, 0, .2)
lines(seq(-10, 10, .01), s(seq(-10, 10, .01)))

```


This could help, as we can evaluate the piecewise function quickly, and use it to filter observations for acceptance or rejection. 
